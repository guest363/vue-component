...
const SVGloaders = {
        ...
        dots: function() {
          const addDomElement = (rootElem, number, color) => {
            ...
            for (let index = 0; index < number; index++) {
             ...
             const position = () => {
               if (number % 2 > 0){
                 return 300 / 2 - (number * 30) + (((number - 1) / 2 * 30) + 15) + 15 + index * 30;
               }
               else return 300 / 2 - (number * 30) + (number  / 2 * 30) + 15 + index * 30;
             };
          ...
          const loaderInit = () => {
            ...
            const animate = () => {
              circle.reduce((latency, c) => {
                c.setAttribute("cy", centreY + 20 * Math.sin(currentAnimationTime + latency));
                return latency + 1;
              }, 0);
          ...
          addDomElement(this.wrapper, this.prop.number, this.prop.color);
          loaderInit();
        }
      }
    };
  ...
  mounted() {
    ...
    this.type[this.prop.type].call(this);
  }
};
